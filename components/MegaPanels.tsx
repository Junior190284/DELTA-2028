...
export function HallOfFame(props:{players:Player[];stats:Record<string,Stat>;trainingStats:Record<string,TrainingStat>;chemistry:Pair[];matches:Match[];trainingSessions?:TrainingSession[];onOpenPlayer:(p:Player)=>void}){
  ...
  return <section className="section v10-hof">
    <div className="v10-page-hero v102-hof-hero devil-card"><div><span className="eyebrow gold">REKORDY DRUŻYNY</span><h2>HALL OF <em>FAME</em></h2><p>Liderzy sezonu, treningów i najlepsza chemia zespołu. Ci, którzy tworzą historię DELTA 2018 GM.</p><div className="v103-hero-badges"><span><b>{played.length}</b> meczów</span><span><b>{goals}</b> goli</span><span><b>{props.chemistry[0]?.score||0}%</b> top chemia</span></div><div className="v103-hero-story"><small>WYRÓŻNIENIE DNIA</small><strong>{records.find(r=>r[1])?.[1]?.display_name||"Pierwszy lider już wkrótce"}</strong></div></div><Trophy size={56}/><blockquote>„Wielcy zawodnicy nie rodzą się sami — tworzy ich drużyna.”</blockquote></div>
